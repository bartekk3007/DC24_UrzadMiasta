...
class PDFGenerator:
    ...
    def render(self, json):
        ...
        body.add_paragraph().add_style("margin-bottom: 25px;").add_text("Urząd miasta Gdańsk")
        ...
        for n, (k, v) in enumerate(json.items(), 1):
            ...
            self.__addParagraph(body, f"{n}. {self.__Paragraphs[k][json["daneOsobowe"]["plec"]] if k in self.__Paragraphs else k}")
    ...
    def __addContent(self, body: pymupdf.Xml, key, val):
        ...
        if type(val) is str:
            ...
            for k, v in val.items():
                self.__addItem(body, f"{self.__Items[k] if k in self.__Items else k}: {v}")
        pass
